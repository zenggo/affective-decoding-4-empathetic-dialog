...
if __name__ == '__main__':
    ...
    # load model
    model = ELMModel(cfg, indexer.n_vocab, indexer.n_special, indexer.n_ctx, indexer, args.beta)
    ...
    # test emotions
    EMOS = ["joyful", "proud", "sad", "angry", "terrified"]
    try:
        for (i, b) in enumerate(data_loader):
            ...
            for c in b['data'][0]['context_text']:
                logstr.append(' - ' + c)
            emos = [b['data'][0]['emotion']] + EMOS
            for e in emos:
                et = torch.tensor([EMOTION_CATES.index(e)], dtype=torch.long)
                resp, _ = gen_BS.generate(et, b['dialog'], b['dialog_state'])
                logstr.append('[%s]: %s' % (e, resp))
            # logging
            logstr = str.join('\n', logstr)
            logger.log(logstr)
